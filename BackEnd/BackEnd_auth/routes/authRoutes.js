...
if (!result.expired) {
  return res.status(403).json({
    ...
    res.cookie("token", token, {
      httpOnly: true,
      secure: process.env.DEPLOY === "production", // bật true nếu chạy HTTPS
      sameSite: "lax",
...
router.post("/register", async (req, res) => {
  try {
    ...
    const newUser = new User({ name,user_name ,email, password, avatar, coverImage });
    await newUser.save();
    // ====== 📌 Cập nhật countUser theo createdAt ======
    const createdAt = newUser.createdAt;
    const month = createdAt.getMonth() + 1; // 0–11 → +1
    const year = createdAt.getFullYear();

    await CountUser.findOneAndUpdate(
      { month, year },
      { $inc: { count: 1 } },
...
router.post("/logout", (req, res) => {
  res.clearCookie("token", {
    httpOnly: true,
    sameSite: "lax",
    secure: process.env.DEPLOY === "production", // đổi thành true nếu dùng HTTPS
  });
